...
var _rawData = randomWorkFlow();
var _autoDataZoomAnimator;

const WorkFlow = () => {
  const chartRef = useRef(null);

  const onDragSwitchClick = (model, api, type) => {
    _draggable = !_draggable;
    const charts = echarts.getInstanceByDom(chartRef.current);
    charts.setOption({
      dataZoom: [
        {
          id: "insideX",
  ...
  const initDrag = () => {
    ...
    chart.on("mousedown", function (param) {
      ...
      _draggingCursorOffset = [
        _draggingEl.x - param.event.offsetX,
        _draggingEl.y - param.event.offsetY,
      ];
      _draggingTimeLength =
    ...
    chart.getZr().on("globalout", dragRelease);
    function dragRelease() {
      _autoDataZoomAnimator.stop();
      if (_draggingEl) {
        chart.getZr().remove(_draggingEl);
        _draggingEl = null;
      }
      if (_dropShadow) {
        chart.getZr().remove(_dropShadow);
        _dropShadow = null;
      }
      _dropRecord = _draggingRecord = null;
    }
    function addOrUpdateBar(el, itemData, style, z) {
      var pointArrival = chart.convertToPixel("grid", [
        itemData.timeArrival,
        itemData.categoryIndex,
      ]);
      var pointDeparture = chart.convertToPixel("grid", [
      ...
      var barHeight =
        Math.abs(
          chart.convertToPixel("grid", [0, 0])[1] -
            chart.convertToPixel("grid", [0, 1])[1]
        ) * HEIGHT_RATIO;
      if (!el) {
      ...
      el.attr({
        ...
        x: pointArrival[0],
        y: pointArrival[1] - barHeight,
      });
      return el;
    }
    function prepareDrop() {
      // Check droppable place.
      var xPixel = _draggingEl.shape.x + _draggingEl.x;
      var yPixel = _draggingEl.shape.y + _draggingEl.y;
      var cursorData = chart.convertFromPixel("grid", [xPixel, yPixel]);
      if (cursorData) {
  ...
  useEffect(() => {
    ...
    chartInstance.setOption({
      ...
      yAxis: {
        ...
        min: 0,
        max: _rawData.program.data.length,
      },
      series: [
        {
          type: "custom",
          renderItem: renderGanttItem,
          dimensions: _rawData.task.dimensions,
          encode: {
            x: [DIM_TIME_ARRIVAL, DIM_TIME_DEPARTURE],
            y: DIM_CATEGORY_INDEX,
            tooltip: [DIM_CATEGORY_INDEX, DIM_TIME_ARRIVAL, DIM_TIME_DEPARTURE],
          },
          data: _rawData.task.data,
        },
        {
          type: "custom",
          renderItem: renderAxisLabelItem,
          dimensions: _rawData.program.dimensions,
          encode: {
            x: -1,
            y: 0,
          },
          data: _rawData.program.data.map(function (item, index) {
  ...
  return (
    <div>
      <div ref={chartRef} style={{ height: "500px", width: "1000px" }}></div>
    </div>
  );
};
...
function randomWorkFlow() {
  ...
  const task = {
    dimensions: ["PID", "StartTime", "EndTime"],
    data: [
      [89, 0, 100],
      [0, 100, 200],
      [1, 50, 150],
